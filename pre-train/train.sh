...
DataPath=${BasePath}/GraphPLM/${dataset}
MODEL=${BasePath}/bart-base

ModelCache=$BasePath/.cache
DataCache=$DataPath/.cache/dump2

lr=3e-5

OUTPUT_DIR=${BasePath}/output/exp.GraphPLM/${dataset}-GraphPLM-bart-base-lr-${lr}-bsz256-debug
...
torchrun --nnodes=1 --nproc_per_node=8 --max_restarts=0 --rdzv_id=1111 --rdzv_backend=c10d main.py \
