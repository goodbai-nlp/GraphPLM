...
from common.constant import special_tokens, arg_to_tokenizer
from common.utils import smart_emb_init
from data_interface.dataset import Seq2SeqDataSet, DataCollatorForSeq2Seq
...
from seq2seq_plm_trainer import Seq2SeqPLMTrainer
...
def main():
    ...
    #     tokenizer.amr_bos_token = "<AMR>"
    #     # tokenizer.amr_bos_token = "<s>"
    #     tokenizer.amr_bos_token_id = tokenizer.convert_tokens_to_ids(tokenizer.amr_bos_token)
    #     tokenizer.amr_eos_token = "</AMR>"
    #     # tokenizer.amr_eos_token = "</s>"
    #     tokenizer.amr_eos_token_id = tokenizer.convert_tokens_to_ids(tokenizer.amr_eos_token)
    ...
    if training_args.smart_init:
        # tokenizer.old_enc_size = ori_tokenizer_size
    ...
    raw_datasets = Seq2SeqDataSet(tokenizer, data_args, model_args)
    
    column_names = raw_datasets.datasets["train"].column_names
    # print("To removed column names:", column_names)
    # column_names = ["amr", "sent"]
    if training_args.do_train:
        ...
        if not os.path.exists(data_args.data_cache_dir + "/train") or data_args.overwrite_cache:
            ...
            train_dataset = load_from_disk(data_args.data_cache_dir + "/train", keep_in_memory=False)
    ...
    if training_args.do_eval:
        ...
        if not os.path.exists(data_args.data_cache_dir + "/valid") or data_args.overwrite_cache:
    ...
    if training_args.do_predict:
        ...
        if not os.path.exists(data_args.data_cache_dir + "/test") or data_args.overwrite_cache:
