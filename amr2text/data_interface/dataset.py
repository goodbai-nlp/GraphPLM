...
class Seq2SeqDataSet(Dataset):
    ...
    def tokenize_function(self, examples):
        ...
        # src = [self.src_prefix + inp for inp in src]                                # Replacing token
        # tgt = [self.tgt_prefix + inp for inp in tgt]

        # for itm in src:
        #     tmp = self.tokenizer.tokenize_amr(itm.split())[:self.max_src_length - 2]
        #     print(tmp)
...
# def padding_func(batch, padding_side="right", pad_token_id=1, key="label", pad_to_multiple_of=1):
#     features = deepcopy(batch)
